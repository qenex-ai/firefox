...
describe("AI Controls telemetry", () => {
  ...
  it("records events when smart window status changes", async () => {
    ...
    let nativeSelect = selectEl.inputEl;
    let blockDialogShown = waitForBlockDialog();

    let blockOptionIndex = Array.from(nativeSelect.children).findIndex(
      option => option.value == "blocked"
    );

    nativeSelect.scrollIntoView();

    let popupShownPromise = BrowserTestUtils.waitForSelectPopupShown(window);
    await EventUtils.synthesizeMouseAtCenter(
      nativeSelect,
      {},
      nativeSelect.ownerGlobal
    );
    let popup = await popupShownPromise;

    let targetItem = Array.from(popup.children).find(
      item => item.value == blockOptionIndex
    );
    ...
    // Verify blocked telemetry event is recorded.
